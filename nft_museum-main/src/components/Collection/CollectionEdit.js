...
import {
    Box, Paper, Button, InputLabel, FilledInput, InputAdornment, TextField
} from '@mui/material';
...
const EditCollectionForm = (props) => {
    ...
    useEffect(() => {
        if (collection) {
            ...
            if (collection.image)
                setLogo({
                    preview: `https://nftapplication.api.mycom.world${collection.image}`,
                })
            if (collection.banner)
                setBanner({
                    preview: `https://nftapplication.api.mycom.world${collection.banner}`,
                })
        }
    }, [collection])
